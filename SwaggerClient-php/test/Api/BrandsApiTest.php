...
class BrandsApiTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        $brand = new Brand(
            [
                ...
                'i18n' => [
                    [
                        'locale' => 'pt_PT',
                        'name'   => 'Brand generated by integration test',
                        'description' => 'asda'
                    ]
                ]
            ]
        ...
        $this->brandsAPI = new Api\BrandsApi(
            new Client(),
            $config
        );

        $brandReturn = $this->brandsAPI->addBrands($brand);

        $this->resourceId = $brandReturn->getData()[0]->getId();
    }
     ...
     *
     */
    public function testAddMinimalBrandsWithSuccess()
    {
        $brand = new Brand(
            [
                'position' => 98,
        ...
        $response = $this->brandsAPI->addBrands($brand);
     ...
     *
     */
    public function testAddBrandsWithError()
    {
        ...
        $this->brandsAPI->addBrands($brand);
    }
     ...
     *
     */
    public function testAddFullBrandsWithSuccess()
    {
        $brand = new Brand();
        $brand->setVisibility([Brand::VISIBILITY_FACEBOOK, Brand::VISIBILITY_MOBILE]);
        $brand->setPosition(98);

        $i18nUS = new I18n();
        $i18nUS->setLocale('en_US');
        $i18nUS->setName('Brand name here');
        $i18nUS->setDescription('Description of brand here');
        ...
        $brand->setI18n([
            ...
            new I18n(
                [
                    'locale' => 'es_ES',
                    'name'   => 'Brand generated by integration test ES',
        ...
        $this->brandsAPI->deleteBrandById($response->getData()[0]->getId());
    }
//
//    //function prepareFileUpload($path = 'image.jpg')
//    //{
...
//    //
//    //    return new UploadedFile($path, $finfo, $mime, null, null, true);
//    //}
//
    /**
     * Test case for addBrands
     ...
     *
     */
    public function testAddFullBrandsWithImagesWithSuccess()
    {
        $brand = new Brand();

        $brand->setVisibility([Brand::VISIBILITY_FACEBOOK, Brand::VISIBILITY_MOBILE]);
        $brand->setPosition(99);
        $i18nUS = new I18n();
        $i18nUS->setLocale('en_US');
        $i18nUS->setName('Brand name here');
        $i18nUS->setDescription('Description of brand here');
        ...
        //$brand->setImageSmall($this->prepareFileUpload('test/Api/image.jpg'));
        $response = $this->brandsAPI->addBrands($brand);

        $this->assertInstanceOf(InlineResponse201::class, $response);

        /**
         * @var $brandCreated Brand
         */
        $brandCreated = $response->getData()[0];

        $this->assertInstanceOf(Brand::class, $brandCreated);

        $this->assertTrue($brandCreated->getActive());

        $this->assertEquals(99, $brandCreated->getPosition());

        $this->assertCount(2, $brandCreated->getVisibility());

        $this->assertInstanceOf(I18n::class, $brandCreated->getI18n()[0]);

        $i18nPt = $brandCreated->getI18n()[0];

        $this->assertEquals('pt_PT', $i18nPt->getLocale());

        $this->assertEquals('Brand generated by integration test', $i18nPt->getName());
    ...
    {
        $brands = $this->brandsAPI->getBrandById(619);

        $this->assertInstanceOf(InlineResponse201::class, $brands);
    }

    /**
     * Test case for deleteBrandById
     *
     * .
     */
    public function testDeleteBrandByIdWithResourceNotFound()
    {
        $this->setExpectedException(ApiException::class, null, 404);

        $this->brandsAPI->deleteBrandById(-1);
    }
    ...
    {
        $brands = $this->brandsAPI->getBrands();

        $this->assertInstanceOf(InlineResponse200::class, $brands);
    }
     ...
     *
     */
    public function testUpdateCategoryById()
    {
        $brandsResponse = $this->brandsAPI->getBrands(null, 1, ['id,desc']);

        $brand = $brandsResponse->getData()[0];

        $brand->setPosition(555);

        $this->brandsAPI->updateBrandsById($brand->getId(), $brand);

        $brandsResponse = $this->brandsAPI->getBrandById($brand->getId());

        $brand = $brandsResponse->getData()[0];

        $this->assertEquals(555, $brand->getPosition());
    }
     ...
     *
     */
    public function testDeleteBrandById()
    {
        $this->brandsAPI->deleteBrandById($this->resourceId);

        $this->setExpectedException(ApiException::class);

        $this->brandsAPI->getBrandById(201);
    }
}
