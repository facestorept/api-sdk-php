 ...
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class CustomersAddressApiTest extends \PHPUnit_Framework_TestCase
{
    private static $config;
    /**
     * @var Api\CustomersAddressApi
     */
    private static $resourceAPI;
    private static $customerAddress;
    private static $customerAddressId;
    ...
    public static function setUpBeforeClass()
    {
        ...
        $customer->setEmail('rqvb111df3d1gww23@gmail.com');
    ...
    /**
     * Test case for addCustomerAddress
     *
     * .
     *
     */
    public function testAddCustomerAddress()
    {
        ...
        $address->setAddress('rua de baixo');
        $address->setCity('Porto');
        $address->setType('Billing');
        $address->setZipCode('81230-401');
        $address->setIsoCodeDistrict('PT-01');

        self::$config = Configuration::getDefaultConfiguration()
            ->setApiKey('APIToken', '083e7be2ca947a899db97d00db4f512db6a85551');

        self::$customerAddress = new CustomersAddressApi(
            new Client(),
            self::$config
        );

        $response = self::$customerAddress->addCustomerAddress(self::$resourceId, $address);

        self::$customerAddressId = $response->getData()[0]->getId();
    }

    /**
     * Test case for getCustomerAddresses
     *
     * .
     *
     */
    ...
    /**
     * Test case for testGetCustomerAddressesById
     *
     * .
     *
     */
    public function testGetCustomerAddressesById()
    {
    ...
    public function testUpdateCustomerAddressById()
    {
        ...
        self::$customerAddress->updateCustomerAddressById(self::$resourceId ,self::$customerAddressId, $address);

        $discountResponse = self::$customerAddress->getCustomersAddressByID(self::$resourceId, self::$customerAddressId);

        $customerAddress = $discountResponse->getData()[0];

        $this->assertEquals('Braga', $customerAddress->getCity());
    }

    /**
     * Test case for testDeleteCustomerAddressById
     *
     * .
     *
     */
    public function testDeleteCustomerAddressById()
    {
        $this->setExpectedException(ApiException::class, null, 404);

        self::$customerAddress->deleteCustomerAddressById(self::$resourceId, self::$customerAddressId);

        self::$customerAddress->getCustomersAddressByID(self::$resourceId, self::$customerAddressId);
    }
}
