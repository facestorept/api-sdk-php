...
class BrandsApiTest extends PHPUnit_Framework_TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        $brand->setVisibility([Model\Brand::VISIBILITY_FACEBOOK]);
        ...
        self::$config = $config = Configuration::getDefaultConfiguration()
            ->setApiKey('APIToken', '083e7be2ca947a899db97d00db4f512db6a85551');

        self::$brandsAPI = new Api\BrandsApi(
            new Client(),
            $config
        );

        $response = self::$brandsAPI->addBrands($brand);
    ...
    public function testAddBrandsWithError()
    {
        ...
        self::$brandsAPI->addBrands($brand);
    }
    ...
    public function testDeleteBrandById()
    {
        ...
        $idToDelete = $response->getData()[0]->getId();

        self::$brandsAPI->deleteBrandById($idToDelete);

        $this->setExpectedException(ApiException::class);

        self::$brandsAPI->getBrandById(201);
    }
    ...
    public function testDeleteBrandByIdWithResourceNotFound()
    {
        ...
        self::$brandsAPI->deleteBrandById(-1);
    }
    ...
    {
        $brands = self::$brandsAPI->getBrandById(self::$resourceId);
    ...
    {
        $brands = self::$brandsAPI->getBrands();
    ...
    {
        $brandsResponse = self::$brandsAPI->getBrands(null, 1, ['id,desc']);

        $brand = $brandsResponse->getData()[0];

        $brand->setPosition(555);

        self::$brandsAPI->updateBrandsById($brand->getId(), $brand);

        $brandsResponse = self::$brandsAPI->getBrandById($brand->getId());
    ...
    {
        self::$brandsAPI->uploadImages(
            self::$resourceId,
            __DIR__ . '/image.jpg',
            __DIR__ . '/image.jpg'
        );

        $brandResponse = self::$brandsAPI->getBrandById(self::$resourceId);
